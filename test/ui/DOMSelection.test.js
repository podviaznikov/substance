...
test.UI("DOM selection spanning over inline at end", function(t) {
  ...
  t.deepEqual(range.start.offset, 8, 'start offset should be extracted correctly.')
  t.deepEqual(range.end.offset, 9, 'start offset should be extracted correctly.')
  t.end()
})
...
test.UI("Issue #376: Wrong selection mapping at end of paragraph", function(t) {
  ...
  t.deepEqual(range.start.path, ['p1', 'content'], 'start path should be correct')
  t.deepEqual(range.start.offset, 2, 'start offset should be correct')
  t.deepEqual(range.end.path, ['p2', 'content'], 'end path should be correct')
  t.deepEqual(range.end.offset, 0, 'end offset should be correct')
  t.end()
})
...
test.UI("Mapping a ContainerSelection from DOM to model", function(t) {
  ...
  t.deepEqual(sel.start.path, ['p1', 'content'], 'start path should be correct.')
  t.equal(sel.start.offset, 1, 'start offset should be correct.')
  t.deepEqual(sel.end.path, ['p2', 'content'], 'end path should be correct.')
  t.equal(sel.end.offset, 2, 'end offset should be correct.')
  t.end()
})
...
test.UI("DOM Coordinate on surface element", function(t) {
  ...
  t.deepEqual(sel.start.path, ['p3', 'content'], 'start path should be correct.')
  t.equal(sel.start.offset, 0, 'start offset should be correct.')
  t.end()
})
